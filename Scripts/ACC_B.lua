...
function Main(tick)
	if not PlayingGame() or Animations.maxCount < 1 or not SleepCheck() then return end	Sleep(Animations.maxCount)
					...
					for k,z in ipairs(entityList:GetProjectiles({target=me})) do
						if z.source then
							if z.source.classId == CDOTA_BaseNPC_Creep_Lane or z.source.classId == CDOTA_BaseNPC_Tower then
								if not (Lasthit.table[me.handle] and Lasthit.table[me.handle].creep) and v.team == me.team and v.visible and v.alive and tick > sleepaggro and v.health > (v.health/100)*5 and GetDistance2D(z.source,me) <= z.source.attackRange + 300 then								
