	...
	if active then
		...
		if hook.level > 0 then
			...
			if victim and victim.visible and hook:CanBeCasted() and SleepCheck("hook") then
				...
				if xyz and config.PredictionGUI and client.mousePosition and GetDistance2D(victim,xyz) > 0 and GetDistance2D(victim,client.mousePosition) <= distxyz+250 then
